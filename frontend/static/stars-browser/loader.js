   ...
   function Emulator(canvas, callbacks, loadFiles) {
     ...
     var start = function (options) {
       ...
       loading.then(function (_game_data) {
                      return new Promise(function(resolve, reject) {
                        ...
                        // Helper to get session key from URL or global
                        function getSessionKey() {
                          // From URL parameter
                          if (typeof URLSearchParams !== 'undefined') {
                            var params = new URLSearchParams(window.location.search);
                            var key = params.get('session');
                            if (key) return key;
                          }
                          // From global (set by stars.html)
                          if (window.ASTRUM_SESSION_KEY) return window.ASTRUM_SESSION_KEY;
                          return null;
                        }

                        var sessionKey = getSessionKey();

                        // Priority 1: WailsFS - DISABLED due to IPC stability issues
                        // TODO: Re-enable once WebSocket IPC connection is stable
                        if (false && sessionKey && typeof WailsFS !== 'undefined' && WailsFS.isAvailable()) {
                          console.log('Using WailsFS backend for session:', sessionKey.substring(0, 20) + '...');
                          var AsyncMirrorFS = BrowserFS.FileSystem.AsyncMirror;

                          new WailsFS({ sessionKey: sessionKey }, function(err, wailsFS) {
                            if (err) {
                              console.warn('WailsFS init failed, falling back to IndexedDB:', err);
                              initWithIndexedDB();
                              return;
                            }

                            // AsyncMirror: InMemory for sync ops, WailsFS for async persistence
                            deltaFS = new AsyncMirrorFS(inMemoryFS, wailsFS);
                            deltaFS.initialize(function(initErr) {
                              if (initErr) {
                                console.warn('AsyncMirror init failed with WailsFS:', initErr);
                                deltaFS = inMemoryFS;
                              }
                              finish();
                            });
                        ...
                        // Priority 2: IndexedDB - Browser storage for standalone use
                        else if (BrowserFS.FileSystem.IndexedDB.isAvailable()) {
                          initWithIndexedDB();
                        }
                        // Priority 3: InMemory only - No persistence
                        else {
                          console.log('Using InMemory filesystem (no persistence)');
                          deltaFS = inMemoryFS;
                          finish();
                        }

                        function initWithIndexedDB() {
                        ...
                        function finish() {
                          ...
                          game_data.fs.initialize(function (e) {
                            if (e) {
                              ...
                              // Function to mount session files as S: drive with write-back sync
                              function mountSessionDrive() {
                                var sessionKey = getSessionKey();
                                if (!sessionKey) {
                                  console.log('No session key - S: drive not mounted');
                                  return Promise.resolve();
                                }

                                // Check if Go bindings are available
                                if (!window.go || !window.go.main || !window.go.main.App || !window.go.main.App.FSListAll) {
                                  console.log('Go bindings not available - S: drive not mounted');
                                  return Promise.resolve();
                                }

                                console.log('Mounting session files as S: drive...');

                                // Create an InMemory filesystem for session files
                                var sessionFS = new BrowserFS.FileSystem.InMemory();

                                // Mount it at /s in the MountableFileSystem
                                var mfs = game_data.fs.getOverlayedFileSystems().readable;
                                mfs.mount('/s', sessionFS);

                                // Track initial file states for sync detection
                                var initialFiles = {}; // path -> size

                                // Recursive function to list all files in sessionFS
                                function listAllFiles(fs, dir, results) {
                                  results = results || [];
                                  try {
                                    var entries = fs.readdirSync(dir);
                                    entries.forEach(function(name) {
                                      var fullPath = dir === '/' ? '/' + name : dir + '/' + name;
                                      try {
                                        var stat = fs.statSync(fullPath);
                                        if (stat.isDirectory()) {
                                          listAllFiles(fs, fullPath, results);
                                        } else {
                                ...
                                // Filter out paths that look like drive letters or recursion
                                // Valid paths: /file.ext, /history/file.ext
                                // Invalid: /c/..., /games/c/..., anything with /c/ in it
                                function isValidSessionPath(path) {
                                  // Skip paths containing drive-like directories
                                  if (/\/[a-z]\//.test(path.toLowerCase())) {
                                    return false;
                                  }
                                  // Skip paths that are too deep (likely recursion)
                                ...
                                // Sync function - detect and upload changed/new files
                                // Uses BrowserFS's fs module to read from /s/ (includes writes from DOSBox)
                                function syncSessionFiles() {
                                  if (!window.go || !window.go.main || !window.go.main.App) {
                                    console.log('Sync skipped - Go bindings not available');
                                    return Promise.resolve();
                                  }

                                  // Get BrowserFS's fs module (same one Emscripten uses)
                                  var bfs = BrowserFS.BFSRequire('fs');
                                  if (!bfs) {
                                    console.log('Sync skipped - BrowserFS not ready');
                                    return Promise.resolve();
                                  }

                                  // List files using BrowserFS fs module
                                  function listFilesRecursive(dir, results) {
                                    results = results || [];
                                    try {
                                      var entries = bfs.readdirSync(dir);
                                      entries.forEach(function(name) {
                                        var fullPath = dir === '/' ? '/' + name : dir + '/' + name;
                                        try {
                                          var stat = bfs.statSync(fullPath);
                                          if (stat.isDirectory()) {
                                  ...
                                  // List files from /s/ (includes writes from DOSBox)
                                  var currentFiles = listFilesRecursive('/s');
                                  var toSync = [];

                                  currentFiles.forEach(function(file) {
                                    // Convert /s/path to /path for comparison and storage
                                    var relativePath = file.path.replace(/^\/s/, '');
                                    if (!relativePath) return; // skip /s itself

                                    // Skip invalid paths (from filesystem corruption)
                                    if (!isValidSessionPath(relativePath)) {
                                      return;
                                    }
                                    var initial = initialFiles[relativePath];
                                    // Sync if file is new or size changed
                                    if (!initial || initial !== file.size) {
                                      toSync.push({ bfsPath: file.path, relativePath: relativePath, size: file.size });
                                    }
                                  });
                                  ...
                                  console.log('Syncing', toSync.length, 'changed files to session dir...');

                                  var syncPromises = toSync.map(function(fileInfo) {
                                    try {
                                      // Read from BrowserFS at /s/path
                                      var data = bfs.readFileSync(fileInfo.bfsPath);
                                      var bytes = new Uint8Array(data);
                                      // Note: FSWriteFile expects raw bytes, Wails handles encoding
                                      return window.go.main.App.FSWriteFile(sessionKey, fileInfo.relativePath, Array.from(bytes), 0x1a4)
                                        .then(function() {
                                          console.log('  Synced S:' + fileInfo.relativePath, '(' + bytes.length + ' bytes)');
                                          // Update initial state after successful sync
                                          initialFiles[fileInfo.relativePath] = bytes.length;
                                        })
                                        .catch(function(err) {
                                          console.warn('  Failed to sync', fileInfo.relativePath + ':', err);
                                        });
                                    } catch (e) {
                                      console.warn('  Failed to read', fileInfo.bfsPath, 'for sync:', e);
                                      return Promise.resolve();
                                    }
                                  });
                                ...
                                // Expose sync function globally for back button and manual sync
                                window.syncSessionFiles = syncSessionFiles;

                                // Fetch session files from Go backend
                                // FSListAll returns [{path: string, isDir: bool, size: int64, mode: int}, ...]
                                return window.go.main.App.FSListAll(sessionKey)
                                  .then(function(entries) {
                                    ...
                                    // Filter out corrupted/recursive paths first
                                    var validEntries = entries.filter(function(e) {
                                      return isValidSessionPath(e.path);
                                    });
                                    if (validEntries.length < entries.length) {
                                      console.warn('Filtered out', entries.length - validEntries.length, 'invalid paths (possible filesystem corruption)');
                                    }

                                    // Filter to only files (not directories) and get paths
                                    // Note: JSON keys are lowercase (path, isDir) from Go struct tags
                                    var files = validEntries.filter(function(e) { return !e.isDir; });
                                    var dirs = validEntries.filter(function(e) { return e.isDir; });

                                    console.log('Session has', files.length, 'files and', dirs.length, 'directories');

                                    // Sort directories by path depth (shorter paths first) to create parents before children
                                    ...
                                    dirs.forEach(function(dir) {
                                      ...
                                      try {
                                        sessionFS.mkdirSync(dirPath, 0x1ff); // 0777
                                        console.log('  Created dir S:' + dirPath);
                                      } catch (e) {
                                    ...
                                    // Load each file
                                    var loadPromises = files.map(function(fileEntry) {
                                      var filePath = fileEntry.path;
                                      return window.go.main.App.FSReadFile(sessionKey, filePath)
                                        .then(function(base64Data) {
                                          ...
                                          // Write to session filesystem
                                          sessionFS.writeFileSync(filePath, new Buffer(bytes), null, flag_w, 0x1a4);
                                          // Track initial state for sync detection
                                          initialFiles[filePath] = bytes.length;
                                          console.log('  Loaded S:' + filePath, '(' + bytes.length + ' bytes)');
                                        })
                                        .catch(function(err) {
                                  ...
                                  .then(function() {
                                    console.log('S: drive mounted successfully');

                                    // Start periodic sync (every 5 seconds)
                                    var syncInterval = setInterval(function() {
                                      syncSessionFiles().catch(function(e) {
                                        console.warn('Periodic sync error:', e);
                                      });
                                    }, 5000);

                                    // Store interval ID for cleanup
                                    window.sessionSyncInterval = syncInterval;
                                    ...
                                    console.log('Session file sync enabled (every 5 seconds)');
                                  })
                                  .catch(function(err) {
                                    console.error('Failed to mount S: drive:', err);
                                  });
                              }
     ...
     var init_module = function(args, fs, locateAdditionalJS, nativeResolution, aspectRatio) {
       return { arguments: args,
                ...
                // Explicitly set keyboard listening element to canvas for Wails webview compatibility
